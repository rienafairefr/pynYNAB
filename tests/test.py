...
class TestGetChangedEntities(CommonTest):
    ...
    def testgetChangedEntities_add(self):
        ...
        self.assertEqual(changed_entities, dict(accounts=[added_account]))
    ...
    def testgetChangedEntities_addtransactionsubtransaction(self):
        ...
        self.client.budget.transactions.append(added_transaction)
        self.client.budget.subtransactions.append(subtransaction1)
        self.client.budget.subtransactions.append(subtransaction2)
        ...
        self.assertEqual('transaction_groups', list(changed_entities.keys())[0])
        transaction_groups = changed_entities['transaction_groups']

        self.assertEqual(1, len(transaction_groups))
        self.assertEqual(added_transaction, transaction_groups[0]['transaction'])

        self.assertIsNotNone(transaction_groups[0]['subtransactions'])
        try:
            self.assertItemsEqual([subtransaction1,subtransaction2], set(transaction_groups[0]['subtransactions']))
        except AttributeError:
            self.assertCountEqual([subtransaction1,subtransaction2], set(transaction_groups[0]['subtransactions']))
    ...
    def testgetChangedEntities_delete(self):
        ...
        self.assertEqual(changed_entities, dict(accounts=[deleted]))

    def testgetChangedEntities_modify(self):
        self.account.account_name = 'BLA'
        changed_entities = self.obj.get_changed_entities()
        self.assertEqual(changed_entities, dict(accounts=[self.account]))
...
class TestUpdateChangedEntities(CommonTest):
    ...
    def testupdateChangedEntities_delete(self):
        ...
        changed_entities = dict(accounts=[self.account2])
        self.client.budgetClient.update_from_changed_entities(changed_entities)
        self.assertEqual(len(self.client.budget.accounts), 0)
...
class OtherTests(CommonTest):
    ...
    def testappend(self):
        ...
        self.assertEqual(list(obj.accounts)[-1], account)
    ...
    def testappendBad(self):
        ...
        def testappend():
            obj.accounts.append(transaction)
