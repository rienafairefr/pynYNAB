...
def setupEmptyClient():
    factory = nYnabClientFactory()
    client = factory.create_client(budget_name='TestBudget',
    ...
    client.catalog.ce_budget_versions.append(budget_version)
    client.budget.be_master_categories.append(master_category)
    client.budget.be_subcategories.append(subcategory)
    client.budget.be_payees.append(payee)
    session.commit()
    client.budget.clear_changed_entities()
    client.catalog.clear_changed_entities()

    client.budgetClient.device_knowledge_of_server = 0
    client.catalogClient.device_knowledge_of_server = 0

    client.budgetClient.current_device_knowledge = 0
    client.catalogClient.current_device_knowledge = 0
